...
/* функция возвращает экспоненту числа при переводе в двоичную форму
экспанента занимает в double 11 бит
в этих 11 разрядах хранится также отрицательные экспоненты.
Для этого подразумевается, что хранимая экспонента - это сдвиг относительно числа 1023.
*/
    int num_order = 0;
